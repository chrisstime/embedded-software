...
static void InitModulesThread(void* pData)
{
  ...
  /*for (uint8_t ledNb = 0; ledNb < NB_LEDS; ledNb++) // 3 semaphores for our LEDs
  {
...
/*lint -restore Enable MISRA rule (6.3) checking. */
{
  ...
  __DI();
  if(TowerStart()) /* initialises everything, check if Flash, LED and tower was started up successfully */
    LEDs_On(LED_ORANGE);
  __EI();
  ...
  //error = OS_ThreadCreate(PacketThread, &PacketSemaphore, &PacketThreadStacks[THREAD_STACK_SIZE - 1],4);
  //error = OS_ThreadCreate(AnalogThread, &AnalogNb0Semaphore, &AnalogThreadStacks[THREAD_STACK_SIZE - 1],5);
	error = OS_ThreadCreate(RxThread, &UARTRxSemaphore, &UARTRxThreadStacks[THREAD_STACK_SIZE - 1],1);
	error = OS_ThreadCreate(TxThread, &UARTTxSemaphore, &UARTTxThreadStacks[THREAD_STACK_SIZE - 1],2);
